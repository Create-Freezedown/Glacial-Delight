...
import static com.pouffydev.glacialdelight.content.block.util.HeaterLevel.FREEZING;
import static com.pouffydev.glacialdelight.content.block.util.HeaterLevel.NONE;
...
public class HeaterBlockEntity extends GDBlockEntity {
    ...
    public static void generalTick(Level level, BlockPos pos, BlockState state, HeaterBlockEntity heater) {
        ...
        boolean isHeaterFrozen = state.getValue(HeaterBlock.frozen);
        if (isHeaterFrozen) {
            level.setBlock(pos, state.setValue(HeaterBlock.heatLevel, FREEZING), 11);
        }
        if (heaterLevel != NONE && fuelTime <= 0) {
            level.setBlock(pos, state.setValue(HeaterBlock.heatLevel, heatLevel).setValue(HeaterBlock.hasFuel, false), 11);
        }
        if (fuelTime > 0) {
            fuelTime = fuelTime - 1;
        }
    ...
    public static void cookingTick(Level level, BlockPos pos, BlockState state, HeaterBlockEntity heater) {
        boolean isHeaterFrozen = state.getValue(HeaterBlock.frozen);
        boolean isLit = state.getValue(StoveBlock.LIT);
        HeaterLevel heaterLevel = state.getValue(heatLevel);
        ...
        } else if (isHeaterFrozen) {
            heater.freezingRecipe();
        } else if (heaterLevel != NONE && heaterLevel != FREEZING) {
    ...
    public static void animationTick(Level level, BlockPos pos, BlockState state, HeaterBlockEntity stove) {
        for(int i = 0; i < stove.inventory.getSlots(); ++i) {
            if (!stove.inventory.getStackInSlot(i).isEmpty() && level.random.nextFloat() < 0.2F) {
                ...
                Direction direction = (Direction)state.getValue(HeaterBlock.facing);
        ...
        if (this.level != null) {
            for(int i = 0; i < this.inventory.getSlots(); ++i) {
                if (!this.inventory.getStackInSlot(i).isEmpty() && this.level.random.nextFloat() < 0.2F) {
                    ...
                    Direction direction = (Direction)this.getBlockState().getValue(HeaterBlock.facing);
                    int directionIndex = direction.get2DDataValue();
                    Vec2 offset = directionIndex % 2 == 0 ? stoveItemVector : new Vec2(stoveItemVector.y, stoveItemVector.x);
